...
| I6 | The chart-G witness pairs are a **ridge**, not two isolated points (**global**, **chart G**, **d=4**) | median likelihood barrier between the two members of a witness pair **8.31 log-units** against a floor barrier of 8.0, with **6 of 13** pairs at or below it, over a set then **3 of 13 refined** under `WIT-02`. Null control - the same criterion over the same number of ordinary prior draws forming no witness pair - has a **minimum** of 339 and a median of 9379. **Superseded by I6b**, which recounts it over the fully refined set | `python scripts/run_g9.py --phases basins` | 13 witness + 13 null paths | :white_check_mark: new, strengthens S-1 |
| I6b | The same ridge, recounted over the pairs that survive refinement (**global**, **chart G**, **d=4**) | **13 of 13 refined** under `WIT-02`, **12 surviving**; median barrier **6.46 log-units**, **0.81 floor units** - 19.2% *below* the floor barrier where the unrefined set was 3.9% above it - with **6 of 12** pairs at or below it, classification **RIDGE** at the registered `ridge_fraction = 1/3`. All thirteen depths reproduce `outputs/g10/ridge_basin.json` bit for bit before the recount. Null control median 7651 | `PYTHONPATH=src python scripts/run_wit02_chartG.py` | 12 witness + 12 null paths, after 13 pairs x 6 refinements | :white_check_mark: new, **closes `WIT-02` for this set** |
...
| J2a | The ridge/basin split is a property of the **dimension**, not the chart | at both `d=16` cells no path below the floor was found (**chart L** 0 of 37, **chart J** 0 of 13) and the only cell putting pairs at the floor is **chart G** at `d=4` (6 of 13). It survives path-length normalisation: 15.35 log-units per unit path against 199 and 569, and 11.45 against the same two when chart G is recounted over its 12 of 13 refined survivors (`docs/CLOSE_ADDENDUM.md`). **Both sides of the comparison are upper bounds**, so the ordering stands; what is untested is whether the *slack* in the bound differs between cells (`PATH-01`) | same | 3 sets, 126 paths | :white_check_mark: new, **withdraws a generation-9 reading** |
