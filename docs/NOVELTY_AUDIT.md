...
| C2 | Identifiability analysis of the I–V → doping map | Burger et al. 2001/2004 establish ill-posedness; SVD analysis exists in the literature | The specific *quantitative* spectrum for these device families is not published, but the method and the qualitative conclusion are known | `scripts/run_identifiability.py`, `run_identifiability_robustness.py` | Measured: 3–4 of 16 dof at the reference conditions; 1–6 (median 3) across 88 measurements; invariant to parameterisation dimension | **Low as method. Moderate as a measured, reproducible artefact.** |
